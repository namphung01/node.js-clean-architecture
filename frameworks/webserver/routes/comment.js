...
export default function commentRouter(express, redisClient) {
  ...
  // POST endpoints
  router.route('/').post([authMiddleware], controller.addNewComment);

  // PUT endpoints
  router.route('/:id').put([authMiddleware], controller.updateComment);

  // DELETE endpoints
  router.route('/:id').delete([authMiddleware], controller.deleteComment);
